...
pub fn path() -> PathBuf {
    ...
    ["data/yc.sqlite", "../scraping_tech/data/yc.sqlite"]
        .iter()
        .map(PathBuf::from)
        .find(|p| p.exists())
        .unwrap_or_else(|| PathBuf::from("data/yc.sqlite"))
}
...
pub fn fetch_pages(conn: &Connection) -> Result<Vec<(String, Option<String>, Option<String>)>> {
    let mut stmt = conn.prepare(
        "SELECT p.url, p.text_content, p.external_links FROM pagedataobjects p
         LEFT JOIN companies c ON p.url = 'https://www.ycombinator.com/companies/' || c.slug
         WHERE p.url LIKE 'https://www.ycombinator.com/companies/%'
         AND p.url NOT LIKE '%/industry/%'
         AND p.url NOT LIKE '%/location/%'
         AND p.url NOT LIKE '%/batch/%'
         AND p.url NOT LIKE '%/tags/%'
