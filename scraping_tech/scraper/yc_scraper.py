...
_client = None
DB_PATH = "/home/vlad/warp/6_Databases/yc.sqlite"
...
def get_unvisited_urls(pattern: Optional[str] = None, exclude: Optional[str] = None, limit: int = 100) -> list[tuple[int, str]]:
    """
    ...
    cursor.execute(f"SELECT id, url FROM websites_from_sitemap WHERE {where} LIMIT ?", params)
...
def count_urls(pattern: Optional[str] = None, exclude: Optional[str] = None, visited: Optional[bool] = None) -> int:
...
def scrape_batch(urls: list[str], wait_time: int = 120):
    """
    Scrape URLs using Desync bulk search.
    
    Args:
        urls: List of URLs to scrape
        wait_time: Max seconds to wait for results
        
    Returns:
        List of PageData objects
    """
    client = get_client()
    return client.simple_bulk_search(
        target_list=urls,
        wait_time=wait_time,
        extract_html=False
    )
...
def run_batch(pattern: Optional[str] = None, exclude: Optional[str] = None, batch_size: int = 100, wait_time: int = 120) -> int:
    """
...
def run_pipeline(pattern: Optional[str] = None, exclude: Optional[str] = None, batch_size: int = 100, max_batches: Optional[int] = None):
    """
...
def stats():
    ...
    for row in cursor.fetchall():
        print(f"  {row['sitemap_source']}: {row['unvisited']} unvisited, {row['visited']} visited")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exclude", type=str, help="URL substring to exclude (e.g., /jobs/)")
